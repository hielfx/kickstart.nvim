-- git-ai Neovim known_human shim (module)
-- Loaded via lua/custom/plugins/git-ai.lua lazy.nvim spec.
...
local state = {
  ...
  pending = {},         -- [repo_root] = { [abs_path] = bufnr }
...
local function find_repo_root(dir)
  local cached = state.repo_root_cache[dir]
  if cached ~= nil then
    return cached or nil
  end
  local result = vim.fn.systemlist { 'git', '-C', dir, 'rev-parse', '--show-toplevel' }
  if vim.v.shell_error ~= 0 or #result == 0 then
    state.repo_root_cache[dir] = false
    return nil
  end
  local root = result[1]
  state.repo_root_cache[dir] = root
  return root
end
...
local function fire(repo_root)
  local pending = state.pending[repo_root] or {}
  state.pending[repo_root] = nil

  local bin = resolve_binary()
  if not bin then
    state.last_error = 'git-ai binary not found (set vim.g.git_ai_bin or $GIT_AI_BIN)'
  ...
  state.last_fired = { repo_root = repo_root, count = #edited_filepaths, at = os.time() }

  local ok, err = pcall(function()
    vim.system(
      { bin, 'checkpoint', 'known_human', '--hook-input', 'stdin' },
      { stdin = payload, text = true, cwd = repo_root },
      function(out)
        if out.code ~= 0 then
          state.last_error = string.format('exit=%d stderr=%s', out.code, (out.stderr or ''):sub(1, 500))
          log('checkpoint failed:', state.last_error)
        else
  ...
  if not ok then
    state.last_error = 'spawn failed: ' .. tostring(err)
    log(state.last_error)
  end
end

local function schedule(repo_root, abs_path, bufnr)
  state.pending[repo_root] = state.pending[repo_root] or {}
  state.pending[repo_root][abs_path] = bufnr

  local timer = state.timers[repo_root]
  if timer then
    timer:stop()
    timer:close()
  end
  timer = uv.new_timer()
  state.timers[repo_root] = timer
  timer:start(
    state.debounce_ms,
    0,
    vim.schedule_wrap(function()
      if state.timers[repo_root] == timer then
        state.timers[repo_root] = nil
      end
...
function M.setup()
  ...
  vim.api.nvim_create_autocmd('VimLeavePre', {
    group = group,
    callback = function()
      for root, timer in pairs(state.timers) do
        if timer and not timer:is_closing() then
          timer:stop()
          timer:close()
        end
  ...
  vim.api.nvim_create_user_command('GitAiStatus', function()
    ...
    if state.last_fired then
      lines[#lines + 1] = string.format(
        '  last    : %d file(s) in %s @ %s',
        state.last_fired.count,
        state.last_fired.repo_root,
        os.date('%H:%M:%S', state.last_fired.at)
      )
    end
    if state.last_error then
      lines[#lines + 1] = '  error   : ' .. state.last_error
    end
  ...
  vim.api.nvim_create_user_command('GitAiCheckpoint', function()
    ...
    state.pending[root] = state.pending[root] or {}
    state.pending[root][abs] = bufnr
    fire(root)
  end, {})
end
